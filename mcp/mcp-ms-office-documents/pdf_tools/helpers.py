...
def parse_inline_formatting_pdf(text):
    """Parse inline markdown formatting for PDF (ReportLab) - returns formatted text."""
...
def process_list_items_pdf(lines, start_idx, is_ordered=False, level=0, max_depth=10):
    ...
    while i < len(lines):
        ...
        # If indentation doesn't match our expected level, stop
        if current_level != level:
            break
